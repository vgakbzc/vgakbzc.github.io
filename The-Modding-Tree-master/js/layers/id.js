var inertiaPerTick = new Decimal(0)
...
addLayer("id", {
    ...
    midsection: [
        ["display-text", function(){
            return "You are getting " + format(inertiaPerTick.mul(20)) + " seconds of Inertia every second."
        }], 
        ["display-text", function(){
    ...
    buyables: {
        11: {
            title: "More Offline Time", // Optional, displayed at the top in a larger font
            ...
            display() { // Everything else displayed in the buyable button after the title
                ...
                Provides you with " + format(data.effect) + " hours of offline time"
            },
    ...
    getInertia() {
        inertiaGenerationLimit = (new Decimal(1.03)).mul(buyableEffect(this.layer, "22"))
        if(getPointGen().lte(inertiaGenerationLimit) && player[this.layer].points.lt(inertiaCap)) {
            inertiaPerTick = new Decimal(0.001)
            inertiaPerTick = inertiaPerTick.mul(buyableEffect(this.layer, "21"))
        } else {
            inertiaPerTick = new Decimal(0)
        }
        player[this.layer].points = player[this.layer].points.add(inertiaPerTick)
    },
})